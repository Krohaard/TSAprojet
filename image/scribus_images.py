...
def load_pictures_name(filepath):
    with open(filepath, "r", encoding="utf-8") as f:
        data = json.load(f)
    repertoire_du_script = pathlib.Path(__file__).resolve()
    common_parts = os.path.commonprefix([repertoire_du_script.parts, pathlib.Path(filepath).parts])
    common_path = pathlib.Path(*common_parts)
    relative_path = './' + str(pathlib.Path(filepath).relative_to(common_path).parts[0]) + '/'
    return [ [group['name'],relative_path + group['file'],group['display']] for group in data ]
...
def generate_header(total_pages=1, page_width=172, page_height=172):
    if total_pages==0:
        total_pages=1
    header = []
...
def generate_xml_from_syllables(syllables_with_colors, start_x=1000, start_y=500, step_y=232, page_width=172, page_height=172):
    """
    ...
    for i, segments in enumerate(syllables_with_colors):
        xpos = start_x
        lol = re.match(r"(.+)\.([^.]+)$",segments[1])
        syllable_text = lol.group(1)  # concaténation des segments
        if pathlib.Path(segments[1]).exists():
            img = Image.open(segments[1])
            kwidth, kheight = img.size
            kdpi = img.info.get("dpi")
            kwidth=kwidth*72/kdpi[0]
            kheight=kheight*72/kdpi[1]
        ...
        xml_lines.append(
            f'        <PAGEOBJECT XPOS="{xpos}" YPOS="{ypos}" OwnPage="{i}" ItemID="{10000000 + i}" PTYPE="2" WIDTH="{page_width}" HEIGHT="{page_height}" '
            f'FRTYPE="0" CLIPEDIT="0" PWIDTH="1" PLINEART="1" PLINEJOIN="128" ANNAME="{syllable_text}" LOCALSCX="{kwidth:.6g}" LOCALSCY="{kwidth:.6g}" LOCALX="0" LOCALY="0" LOCALROT="0" '
            f'PICART="1" SCALETYPE="1" RATIO="1" Pagenumber="{i}" PFILE="{segments[1]}" PRFILE="Embedded GIMP built-in sRGB" IRENDER="0" '
            f'EPROF="Embedded GIMP built-in sRGB" path="M0 0 L{page_width} 0 L{page_width} {page_height} L0 {page_height} L0 0 Z" '
        ...
        xml_lines.append(
            f'        <PAGEOBJECT XPOS="{xpos+212}" YPOS="{ypos}" OwnPage="{i*2}" ItemID="{10000000 + i*2}" PTYPE="4" WIDTH="{page_width}" HEIGHT="{page_height}" '
            f'FRTYPE="0" CLIPEDIT="0" PWIDTH="1" PLINEART="1" ANNAME="{segments[0]}" LOCALSCX="1" LOCALSCY="1" LOCALX="0" LOCALY="0" LOCALROT="0" '
        ...
        xml_lines.append('                <DefaultStyle ALIGN="1" FCOLOR="Registration" LINESPMode="1" FONT="Times New Roman Regular" FONTSIZE="20"/>')

        # Ajout des segments colorés
        for text in segments[2]:
            xml_lines.append(f'                <ITEXT FONT="Times New Roman Regular" FONTSIZE="20" FCOLOR="Registration" BGCOLOR="White" BGSHADE="100" CH="{text}"/>')
            xml_lines.append(f'                <para ALIGN="1" LINESPMode="1"/>')
...
lenght_card=172
height_card=172
xml_content = generate_header(total_pages=len(syllabes_colorees),page_width=lenght_card,page_height=height_card) + "\n"
xml_content += generate_xml_from_syllables(syllabes_colorees,page_width=lenght_card,page_height=height_card) + "\n"
xml_content += generate_footer()

# Export en TXT
with open("images.sla", "w", encoding="utf-8") as f:
    f.write(xml_content)

print("Fichier 'images.sla' généré avec couleurs par segment.")